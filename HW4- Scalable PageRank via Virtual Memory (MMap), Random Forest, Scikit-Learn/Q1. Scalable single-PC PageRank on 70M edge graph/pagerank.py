...
"""
def pagerank(index_file, edge_file, max_node_id, edge_count, damping_factor=0.85, iterations=25):
    index_map = mmap.mmap(
        index_file.fileno(),
        length=4847571*16,  #no of nodes *16
        access=mmap.ACCESS_READ)

    edge_map = mmap.mmap( #bin contains int
        edge_file.fileno(),
        length=68993773*8, 
        access=mmap.ACCESS_READ)
