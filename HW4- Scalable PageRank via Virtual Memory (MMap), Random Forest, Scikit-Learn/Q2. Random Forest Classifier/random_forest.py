...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for i in range(n):
            rnd=np.random.randint(0,high=n-1);
            samples.append(XX[rnd][:-1]);#all but last
            labels.append(XX[rnd][-1]);#last    #check
...
#            print(votes)
            ...
            if len(counts) == 0:
                ...
                if(len(record)!=0):
                    y = np.append(y, self.bootstraps_labels[0][self.bootstraps_datasets[0].index(record.tolist())])
            else:
